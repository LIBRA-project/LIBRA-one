...
floor_cell = openmc.Cell(region=-floor_rpp, fill=Concrete,
                         name='floor')
...
libra_reg, libra_system_cell, materials, src, salt_cell, salt_material, salt_vol = \
        single_wall_model.build_libra_xl(salt_height=breeder_height,
                                salt_thickness=breeder_thickness,
                                reflector_thickness=reflector_thickness)
materials.append(Concrete)
materials.append(air)

outside_air_reg = +floor_rpp.xmin & -floor_rpp.xmax \
                & +floor_rpp.ymin & -floor_rpp.ymax \
                & +floor_rpp.zmax & -boundary_top_plane \
                & ~libra_reg
outside_air_cell = openmc.Cell(region=outside_air_reg,
                            fill=air,
                            name='outside air')

universe = openmc.Universe(cells=[libra_system_cell, 
                                floor_cell,
                                outside_air_cell])
...
tbr_err = np.sqrt(np.sum(np.square(t_tally.get_reshaped_data(value='std_dev').squeeze())))
os.chdir('..')
